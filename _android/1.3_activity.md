...
![Master Activity]({{ site.url }}{{ site.baseurl }}/media/1.3/master_activity.png){: .align-center}
...
![Nuovi files]({{ site.url }}{{ site.baseurl }}/media/1.3/activity_files.png){: .align-center}
...
specifico Android Studio ha deciso di estendere `AppCompatActivity` per garantire retro compatibilità con vecchie
...
**NOTA** Nel caso in cui dimentichi di chiamare `super.onCreate`, l'IDE ti darà un errore e, se provi a
...
_istanziare_ il layout, in altre parole stiamo utilizzando un XML per generare un albero di classi UI (Views). 
...
disabilitare certe funzionalità o cambiare dei parametri della tua app. ;) Un esempio classico è cambiare lo URL del 
server del tuo backend da remoto a locale, nel caso tu voglia far girare il server sul tuo computer durante lo sviluppo 
dell'app! 
{: .notice--success}
...
Prova ad aprire il file e, invece di visualizzarne il sorgente, ti verrà mostrato un rettangolo vuoto.

![Layout Editor]({{ site.url }}{{ site.baseurl }}/media/1.3/layout_editor.png){: .align-center}
...
In alto a destra trovi tre controlli:
...
Personalmente non conosco nessuno che utilizzi la modalità _Design_ perché davvero troppo poco accurata.

Per questo motivo, dalla prossima lezione lavoreremo sempre con l'XML.

## Riassumendo
Per creare un'Activity devi:
- Creare una nuova classe che estenda `Activity`;
